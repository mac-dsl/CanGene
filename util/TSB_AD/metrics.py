...
class metricor:
    ...
    def metric_new(self, label, score, plot_ROC=False, alpha=0.2, ths=None):
        '''input:
        ...
        if ths is None:
            ...
            if np.sum(preds) == 0: 
                ...
                if np.sum(preds) == 0: preds = score > (np.mean(score)+np.std(score))
        else: 
            preds = score > ths
    ...
    def TPR_FPR_RangeAUC(self, labels, pred, P, L):
        ...
        if len(L) == 0:
            existence_ratio = existence / 0.0000001
        else:
    ...
    def RangeAUC(self, labels, score, window=0, percentage=0, plot_ROC=False, AUC_type='window', ths=None):
    ...
    # TPR_FPR_window
    def RangeAUC_volume(self, labels_original, score, windowSize, ths=None):
        score_sorted = -np.sort(-score)
